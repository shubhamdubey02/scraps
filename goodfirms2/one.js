...
async function scrapeGoodFirms() {
  ...
  await page.goto("https://www.goodfirms.co/directory", { waitUntil: "networkidle2" });
  console.log("Navigated to GoodFirms directory page.");

  await page.waitForSelector(".profile-list .company-info");

  const agencies = await page.evaluate(() => {
    return Array.from(document.querySelectorAll(".profile-list .company-info")).map((el) => ({
      name: el.querySelector(".company-title a")?.textContent.trim() || "N/A",
      website: el.querySelector(".website")?.href || "N/A",
      linkedin: "N/A",
      email: "N/A",
      founder: "N/A",
      type: "N/A",
